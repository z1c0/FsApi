...
namespace FsApi
{
    internal class Communicator : ICommunicator
    {
        ...
        public async Task<FsResult<int>> DeleteSession()
        {
            var result = await GetResponse<int>(null, null, Verb.DeleteSession);
            return result;
        }
        ...
        private static string BuildUrl(Verb verb, string command, Dictionary<string, string> args)
        {
            ...
            switch (verb)
            {
                case Verb.CreateSession:
                    ...
                    break;
                case Verb.DeleteSession:
                    sb.Append("DELETE_SESSION");
                    break;

                case Verb.GetNotify:
                    sb.Append("GET_NOTIFIES");
                    break;
